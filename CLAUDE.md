...
| `deck/prompts.generated.ts` (the prompt cards' text) | `node scripts/build-deck-prompts.mjs` — from `prompts/`; `npm run check:deck:prompts` clicks every Copy button and fails on drift |
| `deck/public/shots/turbine-hero.jpg` | `node scripts/build-deck-shot.mjs` — photographs the live festival site |
